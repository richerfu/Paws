...
impl RuntimeUiCacheWriter {
    fn start() -> Self {
        ...
        let worker = std::thread::Builder::new()
            .name("paws-ui-cache-writer".to_owned())
            .spawn(move || loop {
                let pending = {
...
pub(super) fn proxy_test_metadata(url: &str, in_name: &str) -> Result<Metadata, PawsError> {
    let parsed = reqwest::Url::parse(url)
        .map_err(|err| PawsError::Core(format!("invalid proxy test URL: {err}")))?;
    let host = parsed
        .host_str()
        .ok_or_else(|| PawsError::Core("proxy test URL has no host".to_owned()))?
        .to_owned();
...
fn persist_runtime_ui_cache_value(
    ...
    let content = serde_json::to_vec(cache)
        .map_err(|error| PawsError::Core(format!("serialize runtime UI cache failed: {error}")))?;
    fs::write(&temp_path, content)
        .map_err(|error| PawsError::Core(format!("write runtime UI cache failed: {error}")))?;
    fs::rename(&temp_path, path).map_err(|error| {
        let _ = fs::remove_file(&temp_path);
        PawsError::Core(format!("replace runtime UI cache failed: {error}"))
    })
}
...
pub(super) fn platform_ipc_error(error: platform_ipc::PlatformIpcError) -> PawsError {
    PawsError::Core(error.to_string())
}
