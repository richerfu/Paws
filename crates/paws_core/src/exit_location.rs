...
use paws_model::{ExitIpServiceSummary, ExitLocationSnapshot, PawsError};
...
impl<'a> ExitLocationProbe<'a> {
    fn through_proxy(
        ...
        let proxy = reqwest::Proxy::all(proxy_url).map_err(|error| {
            PawsError::Core(format!("invalid exit location proxy URL: {error}"))
        })?;
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|error| {
                PawsError::Core(format!("exit location client setup failed: {error}"))
            })?;
        Ok(Self {
            client,
    ...
    fn through_system_vpn(providers: &'a [ExitLocationProvider]) -> Result<Self, PawsError> {
        let client = reqwest::Client::builder()
            .no_proxy()
            .user_agent(concat!("Paws/", env!("CARGO_PKG_VERSION")))
            .timeout(EXIT_LOCATION_REQUEST_TIMEOUT)
    ...
    async fn run(&self) -> Result<ExitLocationSnapshot, PawsError> {
...
pub(super) async fn probe_exit_location(
    mixed_port: u16,
) -> Result<ExitLocationSnapshot, PawsError> {
