...
pub fn normalize_profile_content(raw_profile: &str) -> Result<String, PawsError> {
...
pub fn normalize_yaml(raw_yaml: &str) -> Result<String, PawsError> {
    let value: Value =
...
pub fn sanitize_profile_for_meow_validation(raw_yaml: &str) -> Result<String, PawsError> {
    let mut value: Value =
        serde_yaml::from_str(raw_yaml).map_err(|err| PawsError::Core(err.to_string()))?;
...
pub fn sanitize_profile_for_meow_validation_at(
    ...
    let mut value: Value =
        serde_yaml::from_str(&sanitized).map_err(|err| PawsError::Core(err.to_string()))?;
    let Some(root) = value.as_mapping_mut() else {
        return Err(PawsError::Core(
            "profile root must be a YAML map or supported proxy subscription".to_owned(),
        ));
...
pub(super) fn subscription_links_to_yaml(content: &str) -> Result<Option<String>, PawsError> {
...
pub(super) fn parse_subscription_link(link: &str) -> Result<Option<Mapping>, PawsError> {
...
pub(super) fn parse_vless_link(link: &str) -> Result<Mapping, PawsError> {
...
pub(super) fn parse_trojan_link(link: &str) -> Result<Mapping, PawsError> {
    let url = Url::parse(link).map_err(subscription_error)?;
    let query = query_map(&url);
    let mut proxy = proxy_base(
...
pub(super) fn parse_ss_link(link: &str) -> Result<Mapping, PawsError> {
    let without_scheme = strip_link_scheme(link, "ss://")
        .ok_or_else(|| PawsError::Core("invalid ss subscription link".to_owned()))?;
    ...
    let (cipher, password) = if let Some(password) = parsed.password() {
        ...
        let decoded = decode_base64_text(&user)
            .ok_or_else(|| PawsError::Core("invalid ss user info".to_owned()))?;
        decoded
            .split_once(':')
            .map(|(cipher, password)| (cipher.to_owned(), password.to_owned()))
            .ok_or_else(|| PawsError::Core("invalid ss cipher/password".to_owned()))?
    };
    let mut proxy = proxy_base(
        fragment
...
pub(super) fn parse_ssr_link(link: &str) -> Result<Mapping, PawsError> {
    let encoded = strip_link_scheme(link, "ssr://")
        .ok_or_else(|| PawsError::Core("invalid ssr subscription link".to_owned()))?
        .trim();
    ...
    let server = parts
        ...
        .ok_or_else(|| PawsError::Core("ssr missing server".to_owned()))?;
    let port = parts
        .next()
        .and_then(|port| port.parse::<u16>().ok())
        .ok_or_else(|| PawsError::Core("ssr missing port".to_owned()))?;
    let protocol = parts
        .next()
        .filter(|value| !value.is_empty())
        .ok_or_else(|| PawsError::Core("ssr missing protocol".to_owned()))?;
    let cipher = parts
        .next()
        .filter(|value| !value.is_empty())
        .ok_or_else(|| PawsError::Core("ssr missing cipher".to_owned()))?;
    let obfs = parts
        .next()
        .filter(|value| !value.is_empty())
        .ok_or_else(|| PawsError::Core("ssr missing obfs".to_owned()))?;
    let password = parts
        .next()
        .and_then(decode_base64_component)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| PawsError::Core("ssr missing password".to_owned()))?;
...
pub(super) fn parse_vmess_link(link: &str) -> Result<Mapping, PawsError> {
    let encoded = strip_link_scheme(link, "vmess://")
        .ok_or_else(|| PawsError::Core("invalid vmess subscription link".to_owned()))?
        .trim();
    let decoded = decode_base64_text(encoded)
        .ok_or_else(|| PawsError::Core("invalid vmess subscription link".to_owned()))?;
    let value: serde_json::Value = serde_json::from_str(&decoded)
        .map_err(|err| PawsError::Core(format!("invalid vmess JSON: {err}")))?;
    let server = json_str_any(&value, &["add", "server", "address", "addr"])
        .ok_or_else(|| PawsError::Core("vmess missing server".to_owned()))?
        .to_owned();
    let port = json_u16_any(&value, &["port"])
        .ok_or_else(|| PawsError::Core("vmess missing port".to_owned()))?;
    let mut proxy = proxy_base(
        json_str_any(
            &value,
...
pub(super) fn parse_hysteria2_link(link: &str) -> Result<Mapping, PawsError> {
    let url = Url::parse(link).map_err(subscription_error)?;
    let query = query_map(&url);
    let mut proxy = proxy_base(
...
pub(super) fn parse_hysteria_link(link: &str) -> Result<Mapping, PawsError> {
    let url = Url::parse(link).map_err(subscription_error)?;
    let query = query_map(&url);
    let mut proxy = proxy_base(
...
pub(super) fn parse_tuic_link(link: &str) -> Result<Mapping, PawsError> {
    let url = Url::parse(link).map_err(subscription_error)?;
    let query = query_map(&url);
    let uuid = decode_component(url.username());
    let password = url.password().map(decode_component).unwrap_or_default();
    if uuid.is_empty() {
        return Err(PawsError::Core("tuic missing uuid".to_owned()));
    }
    if password.is_empty() {
        return Err(PawsError::Core("tuic missing password".to_owned()));
    }
    let mut proxy = proxy_base(
...
pub(super) fn parse_http_link(link: &str) -> Result<Mapping, PawsError> {
    let url = Url::parse(link).map_err(subscription_error)?;
    let query = query_map(&url);
    let mut proxy = proxy_base(
...
pub(super) fn parse_socks5_link(link: &str) -> Result<Mapping, PawsError> {
    let url = Url::parse(link).map_err(subscription_error)?;
    let query = query_map(&url);
    let mut proxy = proxy_base(
...
pub(super) fn proxy_subscription_yaml(mut proxies: Vec<Mapping>) -> Result<String, PawsError> {
    ...
    serde_yaml::to_string(&Value::Mapping(root)).map_err(|err| PawsError::Core(err.to_string()))
}
...
pub(super) fn url_host(url: &Url) -> Result<String, PawsError> {
    url.host_str()
        .map(ToOwned::to_owned)
        .ok_or_else(|| PawsError::Core("subscription link missing host".to_owned()))
}

pub(super) fn url_port(url: &Url) -> Result<u16, PawsError> {
    url.port()
        .ok_or_else(|| PawsError::Core("subscription link missing port".to_owned()))
}
...
pub(super) fn subscription_error(error: url::ParseError) -> PawsError {
    PawsError::Core(format!("invalid subscription link: {error}"))
}
