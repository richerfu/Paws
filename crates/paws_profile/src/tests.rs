...
fn new_store_starts_without_profiles() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("seed")));
...
fn runtime_yaml_merges_rules_and_options() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("runtime")));
...
fn runtime_yaml_sanitizes_app_managed_fields_and_injects_geox_url() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("sanitize")));
    let mut store = ProfileStore::open(root.clone()).unwrap();
...
fn imported_subscription_profile_persists_metadata_and_yaml() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("subscription")));
    let mut store = ProfileStore::open(root.clone()).unwrap();
    let profile_id = store
        .import_profile_content(
            "Remote Demo",
...
fn store_validation_uses_safe_geodata_paths_and_degrades_generated_defaults_when_missing() {
    let root =
        std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("validation-geo")));
...
fn imported_profile_uses_subscription_userinfo_comment() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("comment-info")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
            .import_profile_content(
                "Comment Info",
...
fn subscription_update_interval_marks_due_profiles() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("due-refresh")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content_with_subscription_metadata(
            "Due Demo",
...
fn refresh_success_and_failure_metadata_persist_with_profile() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("refresh-meta")));
    let mut store = ProfileStore::open(root.clone()).unwrap();
    let profile_id = store
        .import_profile_content(
            "Remote Demo",
...
fn imported_base64_share_subscription_is_normalized_to_clash_yaml() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("share-sub")));
    let mut store = ProfileStore::open(root.clone()).unwrap();
    let links = "\
...
fn imported_single_share_link_is_normalized_to_clash_yaml() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("single-share")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
            .import_profile_content(
                "Single",
...
fn ssr_share_link_is_normalized_to_clash_yaml() {
    ...
    assert_eq!(get_string(ssr, "group").as_deref(), Some("Paws"));
...
fn http_and_socks5_share_links_are_normalized_to_clash_yaml() {
    let links = "\
http://user:pass@example.test:8080?allow_insecure=true&headers=User-Agent%3DPaws%3BProxy-Authorization%3DBearer%20token#HTTP%20Proxy
...
";
    ...
    let headers = http
        ...
        .and_then(Value::as_mapping)
        .unwrap();
    assert_eq!(get_string(headers, "User-Agent").as_deref(), Some("Paws"));
    assert_eq!(
...
fn selected_proxy_choices_persist_with_profile() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("select")));
    let mut store = ProfileStore::open(root.clone()).unwrap();
    let profile_id = store
        .import_profile_content(
            "Remote Demo",
...
fn profile_content_can_restore_original_backup_after_edit() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("backup")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "Editable",
...
fn profile_traffic_is_accumulated_in_summary() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("traffic")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "Traffic",
...
fn delete_profile_removes_runtime_and_profile_scoped_provider_cache() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("delete")));
    let mut store = ProfileStore::open(root.clone()).unwrap();
    let profile_id = store
        .import_profile_content(
            "Providers",
...
fn runtime_yaml_uses_default_china_dns_split_policy() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("dns-defaults")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "DNS Defaults",
...
fn updates_vpn_config_in_profile_yaml() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("vpn")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "VPN",
    ...
    let tun = root
        ...
        .and_then(Value::as_mapping)
        .expect("tun");
    assert_eq!(get_bool(paws, "system-proxy"), Some(true));
    assert_eq!(get_bool(paws, "allow-bypass"), Some(true));
...
fn network_ports_and_controller_access_are_profile_scoped_and_validated() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("controller")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "Controller",
...
fn updates_dns_config_in_profile_yaml() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("dns")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "DNS",
...
fn runtime_yaml_replaces_subscription_default_nameserver() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("dns-managed")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "DNS Managed",
...
fn runtime_yaml_removes_subscription_fallback_and_policy_when_unset() {
    let root = std::env::temp_dir().join(format!("paws-profile-test-{}", next_id("dns-clear")));
    let mut store = ProfileStore::open(root).unwrap();
    let profile_id = store
        .import_profile_content(
            "DNS Clear",
