...
pub(crate) fn client_label(value: &str) -> &'static str {
    CLIENT_TYPES
        .iter()
...
pub(crate) fn remote_config_label(value: &str) -> &'static str {
    REMOTE_CONFIGS
        .iter()
        .find(|item| item.value == value)
        .map(|item| item.label)
...
pub(crate) fn build_conversion_url(draft: &SubscriptionConverterDraft) -> Result<String, String> {
    let source = draft
        .source_sub_url
        .replace("\r\n", "\n")
        .replace(['\r', '\n'], "|");
    if source.trim_matches('|').trim().is_empty() {
        return Err("订阅链接不能为空".to_owned());
    }
    if draft.client_type.trim().is_empty() {
        return Err("请选择目标客户端".to_owned());
    }

    let mut output = normalized_backend(&draft.backend)?;
...
pub(crate) fn parse_conversion_url(input: &str) -> Result<SubscriptionConverterDraft, String> {
    let url = Url::parse(input.trim()).map_err(|_| "请输入正确的转换链接".to_owned())?;
    if !matches!(url.scheme(), "http" | "https") {
        return Err("转换链接必须使用 http 或 https".to_owned());
    }
    ...
    let target = param("target").ok_or_else(|| "链接缺少 target 参数".to_owned())?;
    let source = param("url").ok_or_else(|| "链接缺少 url 参数".to_owned())?;
...
pub(crate) async fn resolve_and_parse_conversion_url(
    ...
    if input.is_empty() {
        return Err("转换链接不能为空".to_owned());
    }
    if input.contains("target") {
        return parse_conversion_url(input);
    }
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("解析短链接失败：{err}"))?;
    parse_conversion_url(response.url().as_str())
}

pub(crate) async fn generate_short_url(api: &str, long_url: &str) -> Result<String, String> {
    let api = validate_service_url(api, "短链服务")?;
    ...
    let response = reqwest::Client::new()
        ...
        .send()
        .await
        .map_err(|err| format!("短链接获取失败：{err}"))?;
    let status = response.status();
    let value: Value = response
        .json()
        .await
        .map_err(|err| format!("短链服务返回格式无效：{err}"))?;
    if !status.is_success() {
        return Err(format!("短链服务返回 HTTP {status}"));
    }
...
pub(crate) async fn upload_remote_config(api: &str, content: &str) -> Result<String, String> {
    let api = validate_service_url(api, "配置上传服务")?;
    if content.trim().is_empty() {
        return Err("远程配置内容不能为空".to_owned());
    }
    let response = reqwest::Client::new()
        .post(api)
        ...
        .send()
        .await
        .map_err(|err| format!("远程配置上传失败：{err}"))?;
    let status = response.status();
    let value: Value = response
        .json()
        .await
        .map_err(|err| format!("配置上传服务返回格式无效：{err}"))?;
    if !status.is_success() {
        return Err(format!("配置上传服务返回 HTTP {status}"));
    }
    let uploaded = value
        .get("data")
...
pub(crate) async fn fetch_backend_version(backend: &str) -> Result<String, String> {
    let mut url =
        Url::parse(&normalized_backend(backend)?).map_err(|_| "后端地址无效".to_owned())?;
    ...
    let response = reqwest::get(url)
        .await
        .map_err(|err| format!("后端检测失败：{err}"))?
        .error_for_status()
        .map_err(|err| format!("后端检测失败：{err}"))?;
    let text = response
        .text()
        .await
        .map_err(|err| format!("读取后端版本失败：{err}"))?;
    Ok(text
        .replace("backend\n", "")
...
pub(crate) fn save_draft(draft: &SubscriptionConverterDraft) -> Result<(), String> {
    let path = draft_path().ok_or_else(|| "应用数据目录尚未配置".to_owned())?;
    let parent = path.parent().ok_or_else(|| "应用数据目录无效".to_owned())?;
    fs::create_dir_all(parent).map_err(|err| format!("创建应用数据目录失败：{err}"))?;
    let temp = path.with_extension("json.tmp");
    let bytes =
        serde_json::to_vec_pretty(draft).map_err(|err| format!("序列化转换设置失败：{err}"))?;
    fs::write(&temp, bytes).map_err(|err| format!("保存转换设置失败：{err}"))?;
...
pub(crate) fn clash_install_url(long_url: &str, short_url: &str) -> Result<String, String> {
...
fn normalized_backend(backend: &str) -> Result<String, String> {
    let backend = backend.trim();
    let url = Url::parse(backend).map_err(|_| "请输入正确的后端地址".to_owned())?;
    if !matches!(url.scheme(), "http" | "https") {
        return Err("后端地址必须使用 http 或 https".to_owned());
    }
    if url.host_str().is_none() {
        return Err("后端地址缺少主机名".to_owned());
    }
    if url.fragment().is_some() {
        return Err("后端地址不能包含片段标识".to_owned());
    }
...
fn validate_service_url(input: &str, label: &str) -> Result<Url, String> {
    let url = Url::parse(input.trim()).map_err(|_| format!("{label}地址无效"))?;
    if !matches!(url.scheme(), "http" | "https") || url.host_str().is_none() {
        return Err(format!("{label}地址必须使用 http 或 https"));
    }
    Ok(url)
}
