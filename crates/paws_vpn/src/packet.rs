...
pub(super) fn duplicate_fd(fd: i32) -> Result<i32, PawsError> {
    let duplicated = unsafe { libc::dup(fd) };
    if duplicated < 0 {
        Err(PawsError::Io(std::io::Error::last_os_error().to_string()))
    } else {
        Ok(duplicated)
    }
}

pub(super) fn set_nonblocking(fd: RawFd) -> Result<(), PawsError> {
    let flags = unsafe { libc::fcntl(fd, libc::F_GETFL) };
    if flags < 0 {
        return Err(PawsError::Io(io::Error::last_os_error().to_string()));
    }
    let rc = unsafe { libc::fcntl(fd, libc::F_SETFL, flags | libc::O_NONBLOCK) };
    if rc < 0 {
        Err(PawsError::Io(io::Error::last_os_error().to_string()))
    } else {
        Ok(())
    }
}
