...
impl TryFrom<&str> for RuntimeMode {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        match value {
            ...
            other => Err(PawsError::InvalidMode(other.to_owned())),
        }
    }
}
...
impl TryFrom<&str> for VpnStack {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "lwip" => Ok(Self::Lwip),
            other => Err(PawsError::Core(format!(
...
impl NetworkPortConfig {
    ...
    pub fn validate(self) -> Result<Self, PawsError> {
...
pub fn to_json<T: Serialize>(value: &T) -> Result<String, PawsError> {
    serde_json::to_string(value).map_err(|err| PawsError::InvalidJson(err.to_string()))
}

pub fn from_json<T: for<'de> Deserialize<'de>>(value: &str) -> Result<T, PawsError> {
    serde_json::from_str(value).map_err(|err| PawsError::InvalidJson(err.to_string()))
}
