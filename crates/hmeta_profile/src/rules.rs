...
pub(super) fn parse_imported_rule_lines(rules_text: &str) -> Result<Vec<String>, HMetaError> {
...
pub(super) fn imported_rule_yaml_sequence(value: &Value) -> Result<Vec<String>, HMetaError> {
    match value {
        Value::Sequence(values) => imported_rule_values(values),
        _ => Err(HMetaError::Core(
            "the top-level rules field must be a YAML list".to_owned(),
        )),
    }
}

pub(super) fn imported_rule_values(values: &[Value]) -> Result<Vec<String>, HMetaError> {
    values
        .iter()
        .enumerate()
        .map(|(index, value)| {
...
pub(super) fn validate_imported_rule_line(rule: &str, index: usize) -> Result<(), HMetaError> {
...
pub(super) fn normalize_manual_rule_spec(
    spec: &ManualRuleSpec,
) -> Result<NormalizedManualRule, HMetaError> {
